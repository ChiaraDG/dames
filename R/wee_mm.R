...
#' The function estimates the parameters of interest using weighted estimating equations
#'
...
#' @param inits An optional list of length 3 containing initial values for marginal mean parameters and all dependence parameters. The format of the list should be: (1) estimates of the mean parameters, (2) estimates of the transition parameters (or NULL if only fitting a mLV model) and (3) estimates of the latent variable parameters (or NULL if only fitting a mT model). If NULL, initial values will be automatically generated. Default is NULL
#' @param weights A vector of length nrow(dat) indicating the sampling weights. This is a required argument
#' @param offset an optional offset term. NOT CURRENTLY IMPLEMENTED
...
#' @examples
#' \dontrun{
#' # get the sampling probabilities from a two phase design
#' samp.probs <- ifelse(design$membership == "None", design$sample.probs[1],
#'       ifelse(design$membership == "Some", design$sample.probs[2], design$sample.probs[3]))
#' # estimate the parameters using WEE
#' mod.wee    <- wee_mm(mean.formula = Y ~ X + time + Z + X:time, t.formula = ~1,
#'                     id = id, weights = 1/samp.probs, data = exampledat)
#' summary(mod.wee)
#' }
#'
wee_mm <- function(mean.formula, lv.formula = NULL, t.formula = NULL, id, data, inits = NULL,
                   weights = NULL, offset = NULL, Q = 10, step.max = 1, step.tol = 1e-06, hess.eps = 1e-07,
  ...
  # this are the NSA sampling probabilities. Need to set them to 1 since we are doing IPW
  samp.probs = matrix(c(1,1,1), nrow = length(y), ncol = 3, byrow = TRUE)


  # weights are coded as sampling probabilities, we need to reverse weights
  useROBCOV  <- TRUE
  samp.probi <- matrix(1/weights,nrow=length(y),ncol=1)
  if(is.null(weights)) {
    stop("Sampling weights need to be specified")
  }
  ...
  mm.fit = twophase_cond(params=inits, id=id, X=x, Y=y, Xgam=x.t, Xsig=x.lv, Q=Q, condlike=FALSE,
  ...
  out$control = with(mm.fit, c(condlike, AdaptiveQuad, code, niter, length(table(id)), max(table(id)),
                               useROBCOV, 0))
  names(out$control) <- c('cond.like','adaptive.quad','convergence_code','n_iter',
                          'n_subj','max_n_visit','useRobCov', 'se_comp')
  ...
  if(return_args) attr(out,'args') <- list('mean.formula'=mean.formula, 't.formula'=t.formula,
                                           'lv.formula'=lv.formula,
                                           'id'=id0, 'samp.probs'=samp.probs,'samp.probi'=samp.probi,
                                           'offset'=offset)
